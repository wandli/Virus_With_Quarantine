turtles-own
  [ quarantine?
    susceptible?         ;; These four are states which the agents can take on
    exposed?
    infected?
    recovered?
    infected-time         ;; how long, in weeks, the turtle has been infected
    exposed-time          ;; how long a turtle might be infectious without showing infected
    quarantine-time      ;; how long, in 2 weeks, the turtle has been less infectable
  ]

globals
  [ %infected            ;; what % of the population is infectious
    %max-infected        ;; MAX NUMBER OF IN+IM
    %immune              ;; what % of the population is immune
    %quarantined         ;; what % of the population is quarantine
    chance-reproduce     ;; the probability of a turtle generating an offspring each tick
    exposed-duration     ;; before an exposed turtle become infected it will not be quarantined
    carrying-capacity    ;; the number of turtles that can be in the world at one time
    ]

;; The setup is divided into four procedures
to setup
  clear-all
  setup-constants
  setup-turtles
  update-global-variables
  update-display
  reset-ticks
end

;; We create a variable number of turtles of which 10 are infectious,
;; and distribute them randomly
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      set size 1.2  ;; easier to see
      get-healthy ]
  ask n-of infected-number turtles
    [ get-infected ]
end

;; not change quarantine time and state
to get-infected ;; turtle procedure
  set susceptible? false
  set exposed? false
  set infected? true
  set recovered? false
  set infected-time 0
end

to get-exposed
  set susceptible? false
  set exposed? true
  set infected? false
  set recovered? false
  set exposed-time 0
end

to get-healthy ;; turtle procedure
  set susceptible? true
  set exposed? false
  set infected? false
  set recovered? false
  set infected-time 0
  set quarantine? false
  set quarantine-time 0
end

to get-quarantined ;; turtle procedure
    set quarantine? true
    set quarantine-time quarantine-duration
end

to become-recovered ;; turtle procedure
  set susceptible? false
  set exposed? false
  set infected? false
  set recovered? true
  set quarantine? false
  set quarantine-time 0
end

;; This sets up basic constants of the model.
to setup-constants
  set carrying-capacity 3000
  set chance-reproduce 0    ;; not generate new turtles
  ;; set infected-number 5
  ;; set death-rate 3
  ;; set e2i 80
end

to go
  if %infected = 0 and %quarantined = 0 [ stop ]
  ask turtles [
    get-older
    move
    if infected? or exposed? [ infect ]
    if quarantine2nd and infected? [ quarantine ]
    if quarantine3rd and ( infected? or exposed? ) [ quarantine ]
    ;; if quarantine? and quarantine3rd [ with-quarantine3rd ]
  ]
  update-global-variables
  update-display
  tick
end

to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ infected? ] / count turtles) * 100
      set %immune (count turtles with [ recovered? ] / count turtles) * 100
      if %infected > %max-infected
        [set %max-infected %infected]
      set %quarantined (count turtles with [ quarantine? ] / count turtles) * 100 ]
end

;; IF ISOLATED SET YELLOW
to update-display
  ask turtles
    [ set shape "person"
      if infected? [set color red]
      if recovered? [set color grey]
      if susceptible? [set color green]
      if exposed? [set color yellow ] ]
end

;;Turtle counting variables are advanced.
to get-older ;; turtle procedure
  if quarantine? [
    set quarantine-time quarantine-time - 1
    if quarantine-time = 0 and not infected?
    [ get-healthy ]
  ]
  if exposed? [
    set exposed-time exposed-time + 1
    let q random-float 100
    if q < exposed2infected-chance        ;; the probability of an exposed turtle becoming infected each tick
      [ get-infected ]
    if 100 - q < chance-recover
      [ become-recovered ]
  ]
  if infected? [
    set infected-time infected-time + 1
    if infected-time >= cured-time [ recover-or-die ]
  ]

end

;; Turtles move about at random.
to move ;; turtle procedure
  rt random 100   ;; turn right
  lt random 100
  fd 1
end

;; Thirdary quarantine: quarantine a turtle if it contacts exposed ones.
to with-quarantine3rd ;; turtle procedure
  ask other turtles-here with [ not quarantine? ]
    [ get-quarantined ]
end

;; If a turtle is infected/exposed, it infects other turtles on the same patch.
;; Recovered turtles don't get sick.
to infect ;; turtle procedure
  if not quarantine?
  [
    ask other turtles-here;; with [ susceptible? and not quarantine? ]
      [ if random-float 100 < infectiousness
      [ get-exposed ] ]
  ]
end

to quarantine ;; turtle procedure
  ask other turtles-here with [ not quarantine? ]
    [ get-quarantined ]
end

;; An infected turtle could become infected/dead/recovered in the next tick
to recover-or-die ;; turtle procedure
  let p random-float 100
  if p < chance-recover   ;; either recover or die
    [ become-recovered ]
  if 100 - p < death-rate and infected?
    [ die ]
end

;; If there are less turtles than the carrying-capacity
;; then turtles can reproduce.
to reproduce
  if count turtles < carrying-capacity and random-float 100 < chance-reproduce
    [ hatch 1
      [
        lt 45 fd 1
        get-healthy ] ]
end

to-report death-percentage
  report 1 - count turtles / number-people
end

to startup
  setup-constants ;; so that carrying-capacity can be used as upper bound of number-people slider
end


; Copyright 1998 Uri Wilensky.
; See Info tab for full copyright and license.
@#$#@#$#@
GRAPHICS-WINDOW
660
10
1078
429
-1
-1
10.0
1
10
1
1
1
0
1
1
1
-20
20
-20
20
1
1
1
ticks
30.0

SLIDER
40
155
235
188
quarantine-duration
quarantine-duration
0.0
25
14.0
1.0
1
days
HORIZONTAL

SLIDER
40
121
234
154
chance-recover
chance-recover
0.0
99.0
10.0
1.0
1
%
HORIZONTAL

SLIDER
40
87
234
120
infectiousness
infectiousness
0.0
99.0
80.0
1.0
1
%
HORIZONTAL

BUTTON
62
48
132
83
NIL
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
138
48
209
84
NIL
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
0

PLOT
240
175
610
425
Populations
days
people
0.0
52.0
0.0
200.0
true
true
"" ""
PENS
"infected" 1.0 0 -2674135 true "" "plot count turtles with [ infected? ]"
"recovered" 1.0 0 -7500403 true "" "plot count turtles with [ recovered? ]"
"susceptible" 1.0 0 -10899396 true "" "plot count turtles with [ susceptible? ]"
"total" 1.0 0 -13345367 true "" "plot count turtles"
"exposed" 1.0 0 -1184463 true "" "plot count turtles with [ exposed? ]"

SLIDER
40
10
234
43
number-people
number-people
10
carrying-capacity
1000.0
1
1
NIL
HORIZONTAL

MONITOR
240
76
315
121
NIL
%infected
1
1
11

MONITOR
317
76
391
121
NIL
%immune
1
1
11

MONITOR
393
77
467
122
months
ticks / 30
1
1
11

MONITOR
240
126
325
171
%quarantined
%quarantined
1
1
11

SWITCH
40
305
187
338
quarantine3rd
quarantine3rd
1
1
-1000

SWITCH
40
270
187
303
quarantine2nd
quarantine2nd
1
1
-1000

MONITOR
415
126
507
171
max-infected
%max-infected
2
1
11

INPUTBOX
385
10
480
70
infected-number
10.0
1
0
Number

INPUTBOX
240
10
310
70
death-rate
3.0
1
0
Number

INPUTBOX
315
10
385
70
cured-time
14.0
1
0
Number

SLIDER
40
190
235
223
death-rate
death-rate
0
100
3.0
1
1
%
HORIZONTAL

MONITOR
330
126
410
171
death-rate
death-rate
2
1
11

SLIDER
40
225
235
258
exposed2infected-chance
exposed2infected-chance
0
100
50.0
1
1
%
HORIZONTAL

@#$#@#$#@
## WHAT IS IT?

This model simulates the transmission and perpetuation of a virus in a human population.

It follows the idea of the SEIR model but adds a 'quarantine' factor.


## HOW IT WORKS

The model is initialized with 1000 people, of which 10 are infected.  People move randomly about the world in one of three states: healthy but susceptible (green), sick and infectious (red), not sick but infectious (yellow), and healthy and immune (gray). 

This simulation could model three conditions: no quarantine, secondary quarantine ( isolate all turtles who have interacted with infected ones), and tertiary quarantine (isolate all turtles who have associated with both infected ones and exposed ones).

An infected turtle needs at least 'cured-time' to become cured. It may become recovered at chance 'chance-recover' each day after cured-time. However, an exposed turtle always has a possibility of 'chance-recover' to become recovered. Once got infected, a turtle will die in the rate of 'death-rate'.

An exposed turtle has a possibility, 'exposed2infected-rate', of becoming infected during its exposed-duration. 

A recovered turtle is immune, which means it will never get infected again.

If a turtle is quarantined, it won't infect any other turtles. However, if a turtle is not quarantined before it transfers from exposed to infected, it costs 'accepted-time' days to become quarantined, for others may not know that it is exposed.


The simulation will stop if no one is infected or quarantined.


## HOW TO USE IT

Each 'tick' represents one day in the time scale of this model.

The 'infected-number' is how many turtles are infected initially. 'infectiousness' is how likely an infected/exposed turtle will affect others.

Populations of turtles in all states are shown in the graph 'Populations'.

You can set the variables mentioned above to change the conditions.



## RELATED MODELS
* virus in a network.


## CREDITS AND REFERENCES
Created by Wanda Li.
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.1.1
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
1
@#$#@#$#@
